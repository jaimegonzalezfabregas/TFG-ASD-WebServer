...
app.get('/formulario-aulas', async (req, res) => {
  ...
  if (checkSesion(res, req.url) && req.query.all != 'yes') {
    ...
    if (query.length != 0) {
      ...
      if (actividades_posibles.length != 0) {
        ...
        query_res.forEach((esp) => {
          if (esp.dataValues.edificio != edif) {
            edif = esp.dataValues.edificio;
            espacios_doc.push({ edificio: edif, espacios: []});
          }
          espacios_doc[espacios_doc.length - 1].espacios.push(esp.dataValues);
        });

        console.log(espacios_doc[0].espacios);
        //Enseñamos únicamente los espacios que coincidan con las actividades
        res.render('formulario-aulas', { espacios: espacios_doc, all: false });
        return;
      }
